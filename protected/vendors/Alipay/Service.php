...
class Mb_Pay_Alipay_Service {
	// 线上
	public $gateway_order = "http://wappaygw.alipay.com/service/rest.htm?";
	
	protected $_key; // 安全校验码
	public $mysign; // 签名结果
	public $sign_type; // 签名类型 相当于config文件中的sec_id
	public $parameter; // 需要签名的参数数组
	public $format; // 字符编码格式
	public $req_data = ''; // post请求数据
	 ...
	 */
	public function alipay_wap_trade_create_direct($parameter, $key, $sign_type) {
	 ...
	 */
	public function alipay_Wap_Auth_AuthAndExecute($parameter, $key) {
	 ...
	 * 返回 $data
	 */
	public function post($gateway_url) {
		$ch = curl_init ();
