...
class UserController extends FrontBase
{
	...
	public function ActionJoin()
	{
		...
		if(isset($_POST['username'])){
			...
			$userModel->status = $this->_active_need?-1:1;  //审核状态
			$userModel->groupid = 1;
			$userModel->mobile = $_POST['username'];			
			$userModel->nickname = 'wandermoon用户';
			$userModel->logins = 0;
			$userModel->register_ip = $this->_request->userHostAddress;  //注册ip
			...
			elseif($userModel->save()){
				//$msg = '注册成功';
				//$this->message('success','注册成功', $this->createUrl('user/login'),3);
				Yii::app()->session['is_login']='1';
				Yii::app()->session['username']=$_POST['username'];
				Yii::app()->session['uid']=Yii::app()->db->getLastInsertID();
				//$this->redirect(Yii::app()->homeUrl);
		...
		$testcode='';
		if($get_code==1){
			$testcode = Yii::app()->session['testcode'];
		}

        //获取登录前的URL
        $ret_url = Yii::app()->request->urlReferrer;
		$this->_seoTitle = '注册会员';
		$this->render('join', array('model'=>$model,'msg'=>$msg,'testcode'=>$testcode,'ret_url'=>$ret_url));
	}
	...
	public function ActionReg()
	{
		...
		$sess_code = Yii::app()->session[$mobile];
		if($code!=$sess_code){
		    echo "2";	
		}
		else{
			...
			$userModel->logins = 0;
			$userModel->register_ip = $this->_request->userHostAddress;  //注册ip
			setcookie('username',$mobile,time()+3600*24*7);
	...
	public function ActionSendsms(){
		...
		$sendurl = "http://116.213.72.20/SMSHttpService/send.aspx?username=tongle&password=tongle123&mobile=$mobile&content=【上海同乐旅行】您的注册验证码是$code,请在180秒内输入完成注册&Extcode=&senddate=&batchID=";
		$result=file_get_contents($sendurl);
		Yii::app()->session[$mobile]=$code;
		Yii::app()->session['smscode']=$code;
		echo $result;	
	}
	...
	public function actionLogin()
	{
		...
		if(isset($_POST['username']))
		{
			$username = $_POST['username'];
			$password = $_POST['password'];
			$tmpuser = User::model()->find('username=:username',array('username'=>$username));
			...
			if(!$this->_identity->authenticate()){
				...
				Yii::app()->session['uid']=$tmpuser->uid;
				//cookie
		...
		$this->_seoTitle = '用户登录';
		// display the login form
		$this->render('login',array('msg'=>$msg,'ret_url'=>$ret_url));
	}
	...
	public function actionQuit(){
		...
		$ret_url = $this->createUrl('site/index');				
		$this->redirect($ret_url);
	}
	
	public function actionCheckuser(){
		$mobile = $_POST['mobile'];
		$user = User::model()->findAll('username=:username' , array(':username'=>$mobile));
		if($user){
			echo 1;	
		}
		else{
